...
import { listConfigMaps } from '@/lib/k8s/configmaps';
...
export async function GET(
  ...
  try {
    ...
    const configMaps = await listConfigMaps(kubeconfigString, namespace);

    return NextResponse.json(configMaps);
  } catch (error) {
    console.error('GET /api/clusters/[clusterId]/configmaps error:', error);
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : 'Failed to fetch config maps',
      },
      { status: 500 }
    );
  }
