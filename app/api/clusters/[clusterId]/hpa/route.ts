...
import { listHPA } from '@/lib/k8s/hpa';
...
export async function GET(
  ...
  try {
    ...
    const hpaList = await listHPA(kubeconfigString, namespace);

    return NextResponse.json(hpaList);
  } catch (error) {
    console.error('GET /api/clusters/[clusterId]/hpa error:', error);
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Failed to fetch HPA',
      },
      { status: 500 }
    );
  }
